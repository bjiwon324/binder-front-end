...
export default function FixDetail() {
  ...
  return (
    <>
      <AdminPageBar />
      {isEdit ? (
        <AddBinForm isAdmin={true} binDetail={fixDetail} toggleIsEdit={() => setIsEdit(false)} />
      ) : (
        <AdminDetail state={"수정"} approve={handleAccept} binDetail={fixDetail} toggleIsEdit={() => setIsEdit(true)} />
      )}
